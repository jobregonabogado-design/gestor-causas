  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {!isGmailConnected() ? (
        ...
        <div style={{ background: '#F8F9FC', border: '1px solid #e2e8f0', borderRadius: 10, padding: '14px 16px' }}>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
            ...
            {['AM', 'PM'].map(b => (
              <button key={b} onClick={() => setBloque(b)}
                style={{ padding: '6px 14px', borderRadius: 7, fontSize: 12, fontWeight: 700, border: `1.5px solid ${bloque === b ? '#1E293B' : '#e2e8f0'}`, background: bloque === b ? '#1E293B' : '#fff', color: bloque === b ? '#fff' : '#64748b', cursor: 'pointer', ...f }}>{b}</button>
            ))}
          </div>
