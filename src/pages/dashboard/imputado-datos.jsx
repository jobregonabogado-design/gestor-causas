    ...
    <div style={{border:'1px solid #e2e8f0', borderRadius:14, background:'#fff', boxShadow:'0 1px 2px rgba(15,23,42,0.06)', overflow:'hidden'}}>
      <div
        ...
        <div style={{fontSize:12,color:'#64748b',marginTop:4,...f}}>
          <strong style={{color:'#2F5D48'}}>{numDelitos}</strong> delito{numDelitos!==1?'s':''}
          <span style={{color:'#cbd5e1',margin:'0 8px'}}>·</span>
          <strong style={{color:'#2F5D48'}}>{totalAbonoImp}</strong> días de abono
          <span style={{color:'#cbd5e1',margin:'0 8px'}}>·</span>
      ...
      {expanded && (
        <div style={{padding:'0 16px 16px', borderTop:'1px solid #f1f5f9', display:'flex', flexDirection:'column', gap:16}}>
          ...
          <div style={{marginTop:14}}>
            ...
            {editField==='fecha_nacimiento'?(
              ...
              <div className="fld" onClick={()=>{setEditField('fecha_nacimiento');setEditValue(imp.fecha_nacimiento||'')}}
                style={{padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imp.fecha_nacimiento?'#2F5D48':'#94a3b8',minHeight:38,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
                <span>
                  {fechaDDMM(imp.fecha_nacimiento) || 'Clic para agregar...'}
                  {imp.fecha_nacimiento && (() => {
                    const edad = calcularEdadActual(imp.fecha_nacimiento)
                    return edad !== null ? <span style={{marginLeft:8,fontSize:11,color:'#2F5D48',fontWeight:600,background:'#eff6ff',padding:'1px 7px',borderRadius:10}}>{edad} AÑOS HOY</span> : null
                  })()}
                </span>
            ...
            <div style={{marginTop:14}}>
              ...
              {editField==='fecha_detencion'?(
                ...
                <div className="fld" onClick={()=>{setEditField('fecha_detencion');setEditValue(imp.fecha_detencion||'')}}
                  style={{padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imp.fecha_detencion?'#2F5D48':'#94a3b8',minHeight:38,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
              ...
              <div>
                ...
                {editField==='delegacion_fecha'?(
                  ...
                  <div className="fld" onClick={()=>{setEditField('delegacion_fecha');setEditValue(imp.delegacion_fecha||'')}}
                    style={{padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imp.delegacion_fecha?'#2F5D48':'#94a3b8',minHeight:38,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
          ...
          <div>
            ...
            <select
              style={{width:'100%',padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imp.correo_notificacion?'#2F5D48':'#94a3b8',background:'#fff',cursor:'pointer',...f}}
