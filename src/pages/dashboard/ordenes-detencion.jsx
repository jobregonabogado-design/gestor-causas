  ...
  return (
    <div style={{gridColumn:'1/-1', marginTop:2, marginBottom:2}}>
      ...
      <div
        ...
        style={{cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center', padding:'11px 14px', borderRadius: expanded ? '14px 14px 0 0' : 14, fontSize:13, color:'#2F5D48', minHeight:38, background: vigentes>0 ? '#fef2f2' : '#fff', boxShadow:'0 1px 2px rgba(15,23,42,0.06)', ...f}}>
        ...
        <div style={{background:'#fff',boxShadow:'0 1px 2px rgba(15,23,42,0.06)',borderRadius:'0 0 14px 14px',padding:isMobile?'10px 12px 10px':'14px 16px 12px'}}>
              ...
              <div key={o.id} style={{padding:'10px 0', borderBottom: idx<lista.length-1 ? '1px solid #f1f5f9' : 'none'}}>
                {levantando ? (
                  <div style={{background:'#FAF7F0',border:'1px solid #e2e8f0',borderRadius:10,padding:10}}>
                    ...
                    <input type="date" style={{padding:'7px 9px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} value={fechaLevanta} onChange={e=>setFechaLevanta(e.target.value)}/>
                    <div style={{display:'flex',gap:8,marginTop:8}}>
                      <button onClick={()=>guardarLevantamiento(o)} disabled={!fechaLevanta} style={{background:'#2F5D48',color:'#fff',border:'none',borderRadius:7,padding:'7px 14px',fontSize:12,cursor:'pointer',fontWeight:600,...f}}>✓ Guardar</button>
          ...
          <div style={{paddingTop:12, marginTop: lista.length>0 ? 2 : 0, borderTop: lista.length>0 ? '1px solid #f1f5f9' : 'none'}}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'flex-start'}}>
              <input type="date" style={{flex:'1 1 140px',minWidth:140,padding:'8px 10px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}} value={form.fecha_orden} onChange={e=>setForm(p=>({...p,fecha_orden:e.target.value}))}/>
              <input style={{flex:'1 1 220px',minWidth:180,padding:'8px 10px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}} placeholder="Motivo de la orden..." value={form.motivo} onChange={e=>setForm(p=>({...p,motivo:e.target.value}))}/>
