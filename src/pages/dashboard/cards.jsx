    ...
    <div style={{background:'#FAF7F0',border:'1px solid #e2e8f0',borderRadius:12,padding:'14px 16px',marginBottom:8}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <span style={{fontSize:13,fontWeight:600,color:'#2F5D48',...f}}>{a.tipo||'Audiencia'}</span>
        </div>
...
export function ImputadoCard({ imp, idx, totalImputados, cautelares, ordenesDetencion, esTitular, isMobile, causaId, onAbrirCausaAsociada, onUpdate, onDelete, onGuardarCondena, onVaciarCondena, onGuardarOrdenDetencion, onActualizarOrdenDetencion, onEliminarOrdenDetencion, onRegistrarVisita }) {
  ...
  const f = { fontFamily:"'Manrope','Inter',sans-serif" }
  const inp = { width:'100%', padding:'8px 12px', border:'1.5px solid #e2e8f0', borderRadius:8, fontSize:13, color:'#2F5D48', background:'#fff', ...f }
      ...
      {editField===field?(
        field==='lugar_detencion' ? (
          <div style={{display:'flex',gap:6,alignItems:'flex-start'}}>
            ...
            <button style={{background:'#2F5D48',color:'#fff',border:'none',borderRadius:7,padding:'7px 12px',fontSize:12,cursor:'pointer',...f}} onClick={()=>{onUpdate(field,editValue);setEditField(null)}}>✓</button>
        ...
        <div style={{display:'flex',gap:6}}>
          <input
            ...
            autoFocus/>
          <button style={{background:'#2F5D48',color:'#fff',border:'none',borderRadius:7,padding:'7px 12px',fontSize:12,cursor:'pointer',...f}} onClick={()=>{const v=field==='rut'?formatearRut(editValue):editValue;onUpdate(field,v);setEditField(null);if(field==='rut')buscarPorRut(v)}}>✓</button>
          <button style={{background:'#fff',border:'1.5px solid #e2e8f0',borderRadius:7,padding:'7px 10px',fontSize:12,cursor:'pointer',...f}} onClick={()=>setEditField(null)}>✗</button>
        </div>
        )
      ):(
        <div onClick={()=>{setEditField(field);setEditValue(imp[field]||'')}}
          style={{padding:'8px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imp[field]?'#2F5D48':'#94a3b8',minHeight:36,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
    ...
    <div style={{background:'#FAF7F0',border:'1.5px solid #e2e8f0',borderRadius:14,padding:'18px 20px',marginBottom:14}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
        <div style={{display:'flex',alignItems:'center',gap:8,minWidth:0}}>
          ...
          <div style={{fontSize:14,fontWeight:700,color:'#2F5D48',minWidth:0,...f}}>{imp.nombre||'Sin nombre'}</div>
        </div>
        ...
        <div>
          ...
          {editField==='fecha_nacimiento'?(
            <div style={{display:'flex',gap:6}}>
              ...
              <button style={{background:'#2F5D48',color:'#fff',border:'none',borderRadius:7,padding:'7px 12px',fontSize:12,cursor:'pointer',...f}} onClick={()=>{onUpdate('fecha_nacimiento',editValue);setEditField(null)}}>✓</button>
              <button style={{background:'#fff',border:'1.5px solid #e2e8f0',borderRadius:7,padding:'7px 10px',fontSize:12,cursor:'pointer',...f}} onClick={()=>setEditField(null)}>✗</button>
            </div>
          ):(
            <div onClick={()=>{setEditField('fecha_nacimiento');setEditValue(imp.fecha_nacimiento||'')}}
              style={{padding:'8px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imp.fecha_nacimiento?'#2F5D48':'#94a3b8',minHeight:36,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
              <span>
                {fechaDDMM(imp.fecha_nacimiento) || 'Clic para agregar...'}
                {imp.fecha_nacimiento && (() => {
                  const edad = calcularEdadActual(imp.fecha_nacimiento)
                  return edad !== null ? <span style={{marginLeft:8,fontSize:11,color:'#2F5D48',fontWeight:600,background:'#eff6ff',padding:'1px 7px',borderRadius:10}}>
                    {edad} AÑOS HOY
                  </span> : null
                })()}
              </span>
      ...
      {imp.regimen && (
        <div style={{marginTop:10,display:'flex',alignItems:'center',gap:10}}>
          <div style={{
            ...
            color: imp.regimen==='RPA' ? '#5b21b6' : '#2F5D48',
            ...f
          }}>
          ...
          <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid #fecaca'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8}}>
              <div style={{fontSize:12,fontWeight:600,...f}}>
                {imp.ultima_visita ? (
                  <span style={{color: diasSinVisita>30?'#dc2626':'#2F5D48'}}>👤 Última visita: hace {diasSinVisita} día{diasSinVisita!==1?'s':''} ({fechaDDMM(imp.ultima_visita)})</span>
                ) : (
      ...
      <div style={{marginTop:14,background:'#FAF7F0',border:'1.5px solid #e2e8f0',borderRadius:10,padding:14}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
          <div style={{fontSize:13,fontWeight:600,color:'#2F5D48',...f}}>⚖️ Condena</div>
        ...
        {mostrarFormCondena ? (
          ...
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            <div style={{padding:'10px 14px',background:'#fff',borderRadius:8,border:'1px solid #e2e8f0'}}>
              ...
              <div style={{fontSize:14,fontWeight:700,color:'#2F5D48',...f}}>{fechaDDMM(imp.condena_fecha_inicio)} · {formatearTiempoCondena(imp.condena_anos, imp.condena_meses, imp.condena_dias)}</div>
            </div>
            <div style={{padding:'10px 14px',background:'#fff',borderRadius:8,border:'1px solid #e2e8f0',display:'flex',alignItems:'center',gap:10}}>
              ...
              <div>
                ...
                <div style={{fontSize:16,fontWeight:800,color:'#2F5D48',...f}}>{totalAbono} días</div>
              </div>
            </div>
            <div style={{padding:'10px 14px',background:'#fff',borderRadius:8,border:'1px solid #e2e8f0',display:'flex',alignItems:'center',gap:10}}>
              <span style={{fontSize:18}}>📅</span>
              <div>
                ...
                <div style={{fontSize:16,fontWeight:800,color:'#2F5D48',...f}}>{calcularFechaTerminoCondena(imp.condena_fecha_inicio, imp.condena_anos, imp.condena_meses, imp.condena_dias, totalAbono)}</div>
