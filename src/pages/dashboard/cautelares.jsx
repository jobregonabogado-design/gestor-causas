  ...
  return (
    <div style={{gridColumn:'1/-1',marginTop:2,marginBottom:2}}>
      ...
      <div style={{fontSize:10,color:'#94a3b8',textTransform:'uppercase',letterSpacing:1.5,marginBottom:6,fontWeight:600,...f}}>
        Cautelar Personal{nombreImputado && <span style={{color:'#2F5D48',textTransform:'none',letterSpacing:0}}> — {nombreImputado}</span>} {esRPA && <span style={{color:'#7c3aed'}}>· RPA</span>}
      </div>
      ...
      <div
        ...
        style={{
          ...
          color:'#2F5D48', minHeight:38, background:'#fff', boxShadow:'0 1px 2px rgba(15,23,42,0.06)', ...f,
        }}>
      ...
      {expanded && (
        <div style={{background:'#fff',boxShadow:'0 1px 2px rgba(15,23,42,0.06)',borderRadius:'0 0 14px 14px',padding:isMobile?'10px 12px 10px':'14px 16px 12px'}}>
              ...
              <div key={ct.id} style={{padding:'10px 0', borderBottom: idx<cautelares.length-1 ? '1px solid #f1f5f9' : 'none'}}>
                {cerrando ? (
                  <div style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:10,padding:10}}>
                    ...
                    <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                      <div style={{display:'flex',flexDirection:'column',gap:2}}>
                        ...
                        <input type="date" style={{padding:'6px 8px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} value={cierreForm.fecha_inicio} onChange={e=>setCierreForm(p=>({...p,fecha_inicio:e.target.value}))}/>
                      </div>
                      <div style={{display:'flex',flexDirection:'column',gap:2}}>
                        <label style={{fontSize:9,color:'#94a3b8',...f}}>Término de cautelar</label>
                        <input type="date" style={{padding:'6px 8px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} value={cierreForm.fecha_termino} onChange={e=>setCierreForm(p=>({...p,fecha_termino:e.target.value}))}/>
                      </div>
                    </div>
                  ...
                  <div style={{background:'#FAF7F0',border:'1px solid #e2e8f0',borderRadius:10,padding:10}}>
                    <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                      <select style={{flex:'1 1 200px',minWidth:160,padding:'7px 9px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} value={editForm.tipo} onChange={e=>setEditForm(p=>({...p,tipo:e.target.value}))}>
                        {TIPOS.map(t=><option key={t}>{t}</option>)}
                      </select>
                      <div style={{display:'flex',flexDirection:'column',gap:2}}>
                        <label style={{fontSize:9,color:'#94a3b8',...f}}>Inicio</label>
                        <input type="date" style={{padding:'6px 8px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} value={editForm.fecha_inicio} onChange={e=>setEditForm(p=>({...p,fecha_inicio:e.target.value}))}/>
                      </div>
                      <div style={{display:'flex',flexDirection:'column',gap:2}}>
                        <label style={{fontSize:9,color:'#94a3b8',...f}}>Término (opcional)</label>
                        <input type="date" style={{padding:'6px 8px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} value={editForm.fecha_termino} onChange={e=>setEditForm(p=>({...p,fecha_termino:e.target.value}))}/>
                      </div>
                    </div>
                    <div style={{marginTop:8}}>
                      ...
                      <input style={{width:'100%',padding:'7px 9px',border:'1.5px solid #fecaca',borderRadius:8,fontSize:12,color:'#2F5D48',background:'#fff',...f}} placeholder="Ej: Error de tipeo en la fecha, tipo mal seleccionado..." value={motivoEdit} onChange={e=>setMotivoEdit(e.target.value)}/>
                    </div>
                    <div style={{display:'flex',gap:8,marginTop:8}}>
                      <button onClick={guardarEdicion} disabled={!editForm.fecha_inicio} style={{background:'#2F5D48',color:'#fff',border:'none',borderRadius:7,padding:'7px 14px',fontSize:12,cursor:'pointer',fontWeight:600,...f}}>✓ Guardar</button>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:6}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:600,color:'#2F5D48',display:'flex',alignItems:'center',gap:6,flexWrap:'wrap',...f}}>
                      {ct.tipo}
                ...
                {!editando && esDirecto && (
                  <div style={{marginTop:6,fontSize:12,color:'#2F5D48',...f}}>
            ...
            <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300,backdropFilter:'blur(2px)'}}
              ...
              <div style={{background:'#fff',borderRadius:16,padding:20,width:340,maxWidth:'90vw',boxShadow:'0 20px 60px rgba(15,23,42,0.2)'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                  <div style={{fontSize:13,fontWeight:700,color:'#2F5D48',...f}}>🧮 Abono a otra fecha</div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  <input type="date" style={{width:'100%',padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}} value={fechaCalc} onChange={e=>setFechaCalc(e.target.value)}/>
                  {fechaCalc !== hoyStr && (
                  ...
                  <div style={{background:'#FAF7F0',borderRadius:10,padding:'12px 14px',fontSize:13,color:'#475569',...f}}>
                    Abono proyectado: <strong style={{color:'#2F5D48',fontSize:16}}>{cautelares.reduce((s,ct)=>{
          ...
          <div style={{paddingTop:12, marginTop: cautelares.length>0 ? 2 : 0, borderTop: cautelares.length>0 ? '1px solid #f1f5f9' : 'none'}}>
            ...
            <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
              <select style={{flex:'1 1 220px',minWidth:180,padding:'8px 10px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}} value={form.tipo} onChange={e=>setForm(p=>({...p,tipo:e.target.value}))}>
                {TIPOS.map(t=><option key={t}>{t}</option>)}
              </select>
              <input type="date" style={{flex:'1 1 140px',minWidth:140,padding:'8px 10px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}} value={form.fecha_inicio} onChange={e=>setForm(p=>({...p,fecha_inicio:e.target.value}))}/>
              {form.tipo==='Firma' && (
                <select style={{flex:'1 1 140px',minWidth:140,padding:'8px 10px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}} value={form.frecuencia} onChange={e=>setForm(p=>({...p,frecuencia:e.target.value}))}>
