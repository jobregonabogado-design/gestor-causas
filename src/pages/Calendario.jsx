...
const CSS = `
  ...
  .btn-blue { font-family:'Manrope','Inter',sans-serif; background:#2F5D48; color:#fff; border:none; border-radius:10px; padding:9px 20px; font-size:13px; font-weight:600; cursor:pointer; transition:background 0.25s ease, box-shadow 0.25s ease; box-shadow:0 2px 8px rgba(30,58,95,0.2); text-transform:uppercase; letter-spacing:0.3px; }
  .btn-blue:hover { background:#1e40af; box-shadow:0 4px 16px rgba(30,58,95,0.3); }
  .btn-out { font-family:'Manrope','Inter',sans-serif; background:#fff; color:#374151; border:1.5px solid #e5e7eb; border-radius:10px; padding:8px 18px; font-size:13px; font-weight:500; cursor:pointer; transition:border-color 0.25s ease, color 0.25s ease, background 0.25s ease; text-transform:uppercase; letter-spacing:0.3px; }
  .btn-out:hover { border-color:#93c5fd; color:#2F5D48; background:#f8faff; }
  input,select,textarea { font-family:'Manrope','Inter',sans-serif !important; text-transform:uppercase; }
...
function AudienciaEditCard({ a, onDelete, onUpdate, f, isMobile }) {
  ...
  const inp = { width:'100%', padding:'7px 10px', border:'1.5px solid #e2e8f0', borderRadius:7, fontSize:12, color:'#2F5D48', background:'#fff', ...f }
    ...
    <div className="aud-card" style={{background:'#fff',border:`1.5px solid ${c.border}`,borderRadius:12,padding:'14px 16px',boxShadow:'0 1px 4px rgba(15,23,42,0.05)',marginBottom:8}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
        ...
        <div style={{display:'flex',gap:6,alignItems:'center'}}>
          <span style={{fontSize:12,fontWeight:700,color:'#2F5D48',...f}}>📅 {(()=>{const p=(a.fecha||'').split('-');return p.length===3?`${p[2]}-${MESES[parseInt(p[1],10)-1]?.substring(0,3)||p[1]}`:(a.fecha||'—')})()}</span>
...
export default function Calendario({ onVerCausa, abrirGmailAlEntrar }) {
  ...
  const inp = { width:"100%", padding:"9px 12px", border:"1.5px solid #e5e7eb", borderRadius:7, fontSize:13, color:"#2F5D48", background:"#fff", ...f }
    ...
    <div style={{background:"#FAF7F0",minHeight:"100vh",...f}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:isMobile?"14px":"28px"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:12}}>
          <div>
            <h1 style={{fontSize:isMobile?20:26,fontWeight:800,color:"#2F5D48",margin:0,letterSpacing:"-0.5px"}}>Calendario de audiencias</h1>
            <p style={{fontSize:14,color:"#64748b",marginTop:4}}><span style={{fontWeight:700,color:"#2F5D48"}}>{audDelMes.length}</span> audiencias en {MESES[mes]} {anio}</p>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            <button className="btn-blue" onClick={()=>setShowForm(true)}>+ Nueva audiencia</button>
            <button onClick={handleGmailToggle} style={{background:showGmail?'#2F5D48':'#fff',color:showGmail?'#fff':'#475569',border:'1.5px solid #e2e8f0',borderRadius:8,padding:'8px 16px',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:"'Manrope','Inter',sans-serif",transition:'all 0.2s',textTransform:'uppercase',letterSpacing:0.3}}>
              📧 Gmail
            </button>
            <button onClick={()=>setVistaLista(!vistaLista)} className="btn-out"
              style={{background:vistaLista?"#2F5D48":"#fff",color:vistaLista?"#fff":"#475569",borderColor:vistaLista?"#2F5D48":"#e2e8f0"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4,background:"#fff",border:"1.5px solid #e2e8f0",borderRadius:10,padding:"4px"}}>
              ...
              <span style={{fontWeight:700,fontSize:15,minWidth:170,textAlign:"center",color:"#2F5D48",...f}}>{MESES[mes]} {anio}</span>
              <button onClick={()=>navMes(1)} className="nav-btn" style={{background:"transparent",border:"none",borderRadius:7,width:36,height:36,cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",color:"#475569"}}>›</button>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:20,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
            ...
            <div style={{background:"#fff",borderRadius:16,border:"1px solid #e2e8f0",overflowX:isMobile?"visible":"auto",WebkitOverflowScrolling:"touch",boxShadow:"0 1px 8px rgba(15,23,42,0.06)"}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",background:"#2F5D48",minWidth:isMobile?0:500}}>
                {DIAS.map((d,i)=>(
                  ...
                  return(
                    <div key={dia} className="cal-day" onClick={()=>setSelDia(dia===selDia?null:dia)}
                      ...
                      <div style={{display:"flex",justifyContent:isMobile?"center":"flex-end",marginBottom:isMobile?2:5}}>
                        <div style={{width:isMobile?22:30,height:isMobile?22:30,borderRadius:"50%",background:esHoy?"#2F5D48":"transparent",color:esHoy?"#fff":esFind?"#94a3b8":"#1e293b",fontSize:isMobile?11:14,fontWeight:esHoy?700:600,display:"flex",alignItems:"center",justifyContent:"center",...f}}>{dia}</div>
                      </div>
                      {isMobile ? (
                        auds.length>0 && (
            ...
            <div>
              {selDia&&audDelDia.length>0?(
                <div>
                  <div style={{fontSize:15,fontWeight:700,color:"#2F5D48",marginBottom:12,...f}}>{selDia} de {MESES[mes]} — <span style={{color:"#2563eb"}}>{audDelDia.length} audiencia{audDelDia.length>1?"s":""}</span></div>
                ...
                <div style={{background:"#FAF7F0",borderRadius:14,padding:24,textAlign:"center",border:"1.5px dashed #e2e8f0"}}>
                ...
                <div>
                  <div style={{fontSize:15,fontWeight:700,color:"#2F5D48",marginBottom:12,...f}}>Próximas — {MESES[mes]}</div>
                  <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:580,overflowY:"auto"}}>
                  ...
                  return(
                    <tr key={i} style={{borderBottom:"1px solid #f1f5f9",background:i%2===0?"#fff":"#fafafa"}}>
                      <td style={{padding:"11px 16px",...f}}>
                        {a.ruc && onVerCausa ? (
                          <span onClick={()=>irACausa(a.ruc)} style={{fontSize:12,fontWeight:700,color:'#2F5D48',cursor:'pointer',textDecoration:'underline',...f}}>{a.ruc}</span>
                        ) : (
      ...
      {showForm&&(
        <div style={{position:"fixed",top:0,left:0,width:"100%",height:"100%",background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",paddingTop:isMobile?"4vh":"8vh",paddingBottom:"4vh",zIndex:200,backdropFilter:"blur(3px)"}}
          ...
          <div style={{background:"#fff",border:"1px solid #eaecf4",borderRadius:14,padding:isMobile?20:32,width:520,maxWidth:"90vw",boxShadow:"0 20px 60px rgba(15,23,42,0.18)"}}>
            <div style={{fontFamily:"'Plus Jakarta Sans',serif",fontSize:20,fontWeight:700,color:"#2F5D48",marginBottom:22}}>Nueva Audiencia</div>
