...
export default function Dashboard({ session }) {
                ...
                <div style={{gridColumn:'1/-1',marginTop:8}}>
                  ...
                  {(c.imputado||'').split('|').filter((_,i)=>i>0).map((imp,i)=>(
                    <div key={i} style={{display:'flex',gap:8,marginBottom:6,alignItems:'center'}}>
                      ...
                      <button onClick={()=>{const imps=(c.imputado||'').split('|');imps.splice(i+1,1);updateField('imputado',imps.join('|'))}} style={{background:'#fef2f2',border:'1px solid #fecaca',borderRadius:7,padding:'7px 10px',fontSize:12,color:'#dc2626',cursor:'pointer',...f}}>✕</button>
                    </div>
                  ))}
  ...
  return(
    <div style={{background:'#f8fafc',minHeight:'100vh',...f}}>
      ...
      {/* MODAL NUEVA CAUSA */}
      {showNuevaCausa&&(
        <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1001,backdropFilter:'blur(4px)',position:'fixed',top:0,left:0,right:0,bottom:0}} onClick={e=>e.target===e.currentTarget&&setShowNuevaCausa(false)}>
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:16,padding:32,width:540,maxWidth:'90vw',boxShadow:'0 24px 80px rgba(0,0,0,0.2)',maxHeight:'90vh',overflowY:'auto',position:'relative',zIndex:1002}}>
