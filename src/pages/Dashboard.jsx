...
const CSS = `
  ...
  .tab-btn:hover { color:#2F5D48 !important; }
  ...
  .sort-col:hover { color:#2F5D48 !important; }
  .btn-primary { font-family:'Manrope','Inter',sans-serif; background:#2F5D48; color:#fff; border:none; border-radius:10px; padding:9px 20px; font-size:13px; font-weight:600; cursor:pointer; transition:background 0.25s ease, box-shadow 0.25s ease; box-shadow:0 2px 8px rgba(30,58,47,0.2); text-transform:uppercase; letter-spacing:0.3px; }
  .btn-primary:hover { background:#1E3A2F; box-shadow:0 4px 16px rgba(30,58,47,0.3); }
  ...
  .btn-secondary:hover { border-color:#93c5fd; color:#2F5D48; background:#f8faff; }
...
export default function Dashboard({ session, userRol, registrarActividad, causaInicial, onCausaInicialUsada, showStats, setShowStats }) {
  ...
  const chartResultados=useMemo(()=>{
    ...
    return Object.entries(map).sort((a,b)=>b[1]-a[1]).map(([subestado,value])=>({
      ...
      bg: estadoConfig[subestado]?.bg || '#FAF7F0',
  ...
  if(view==='detail'&&selectedCausa){
    ...
    return(
      <div style={{background:'#FAF7F0',minHeight:'100vh',...f}} className="detail-enter">
        <style>{CSS}</style>
        <div style={{maxWidth:1060,margin:'0 auto',padding:'24px 28px'}}>
          ...
          <div style={{padding:'28px 28px 20px',borderRadius:'20px 20px 0 0'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:12}}>
              <div>
                <div style={{fontSize:22,fontWeight:800,color:'#2F5D48',marginBottom:6,letterSpacing:'-0.5px',...f}}>RUC <span style={{color:'#2F5D48'}}>{c.ruc}</span></div>
                <div style={{fontSize:13,color:'#94a3b8',display:'flex',gap:16,flexWrap:'wrap',alignItems:'center',...f}}>
                  ...
                  {imputados.length === 1 && imputados.filter(i=>i.regimen).map(i=>(
                    <span key={i.id} style={{
                      ...
                      color:i.regimen==='RPA'?'#5b21b6':'#2F5D48',...f
                    }}>{i.regimen}</span>
                  ))}
                </div>
              </div>
          ...
          {(() => {
              ...
              return (
                <div style={{background:'#fff',padding:'10px 16px'}}>
                  <select value={activeTab} onChange={e=>setActiveTab(e.target.value)}
                    style={{width:'100%',padding:'10px 12px',border:'1.5px solid #e2e8f0',borderRadius:10,fontSize:13,fontWeight:600,color:'#2F5D48',background:'#FAF7F0',...f}}>
            ...
            return (
              <div style={{background:'#fff',display:'flex',overflowX:'auto',padding:'0 20px'}}>
                {tabsList.map(([k,l])=>(
                  <button key={k} className="tab-btn" onClick={()=>setActiveTab(k)} style={{padding:'13px 16px',fontSize:13,fontWeight:activeTab===k?600:400,color:activeTab===k?'#2F5D48':'#94a3b8',borderBottom:`2px solid ${activeTab===k?'#2F5D48':'transparent'}`,whiteSpace:'nowrap',marginBottom:0}}>{l}</button>
                ))}
              </div>
            )
          })()}
          <div style={{background:'#fff',padding:isMobile?16:28,borderRadius:'0 0 20px 20px'}}>
              ...
              <div className="grid2-mobile" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                ...
                <div style={{gridColumn:'1/-1',marginBottom:2}}>
                  ...
                  <div title="Se edita desde la pestaña Imputado — acá solo se muestra"
                    style={{padding:'11px 14px',border:'none',borderRadius:14,fontSize:13,color:c.imputado?'#2F5D48':'#94a3b8',minHeight:38,display:'flex',alignItems:'center',background:'#FAF7F0',...f}}>
                ...
                <div style={{gridColumn:'1/-1',marginBottom:2}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6,gap:10,flexWrap:'wrap'}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 14px',border:'none',boxShadow:'0 1px 2px rgba(15,23,42,0.06)',borderRadius:20,fontSize:12,color: getCorteApelaciones(c.tribunal) ? '#2F5D48' : '#94a3b8',background:'#fff',fontWeight:600,minWidth:0,maxWidth:'100%',...f}}>
                          ...
                          editField===fieldKey ? (
                            <span style={{display:'inline-flex',gap:4,alignItems:'center'}}>
                              <input type={tipo} autoFocus value={editValue} onChange={e=>setEditValue(e.target.value)}
                                ...
                                style={{width:tipo==='date'?130:110,padding:'4px 8px',border:'1.5px solid #c4b5fd',borderRadius:7,fontSize:12,color:'#2F5D48',background:'#fff',...f}}/>
                            </span>
                          ) : (
                            <span onClick={()=>{setEditField(fieldKey);setEditValue(valor||'')}}
                              style={{cursor:'pointer',color:valor?'#2F5D48':'#a78bfa',fontWeight:valor?600:500,...f}}
                ...
                <details className="seccion-plegable" open={!isMobile} style={{gridColumn:'1/-1',border:'1px solid #e2e8f0',borderRadius:16,padding:'18px 20px',background:'#FAF7F0'}}>
                    ...
                    <div style={{flex:'1 1 150px',minWidth:140}}>
                      ...
                      {editField==='fecha_hechos'?(
                        <div style={{display:'flex',gap:4}}>
                          ...
                          <input type="date" style={{width:'100%',padding:'7px 9px',border:'none',borderRadius:8,fontSize:11,color:'#2F5D48',background:'#fff',boxShadow:'0 1px 2px rgba(15,23,42,0.06)',...f}}
                ...
                {imputados.length <= 1 ? (
                  <details className="seccion-plegable" open={!isMobile} style={{gridColumn:'1/-1',border:'1px solid #e2e8f0',borderRadius:16,padding:'18px 20px',background:'#FAF7F0',marginTop:2}}>
                    <summary style={{fontSize:10,color:'#94a3b8',textTransform:'uppercase',letterSpacing:1.3,fontWeight:700,marginBottom:14,display:'flex',justifyContent:'space-between',alignItems:'center',...f}}>
                      ...
                      <div>
                        ...
                        {editField==='fecha_detencion'?(
                          <div style={{display:'flex',gap:6}}>
                            <input type="date" style={{width:'100%',padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:'#2F5D48',background:'#fff',...f}}
                              value={editValue} onChange={e=>setEditValue(e.target.value)}
                          ...
                          <div className="fld" onClick={()=>{setEditField('fecha_detencion');setEditValue(imputados[0].fecha_detencion||'')}}
                            style={{padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:imputados[0].fecha_detencion?'#2F5D48':'#94a3b8',minHeight:38,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
                        ...
                        <div>
                          ...
                          {editField==='delegacion_fecha'?(
                            ...
                            <div className="fld" onClick={()=>{setEditField('delegacion_fecha');setEditValue((imputados.length===0?c.delegacion_fecha:imputados[0].delegacion_fecha)||'')}}
                              style={{padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:(imputados.length===0?c.delegacion_fecha:imputados[0].delegacion_fecha)?'#2F5D48':'#94a3b8',minHeight:38,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',background:'#fff',...f}}>
                    ...
                    <div style={{gridColumn:'1/-1',marginTop:4}}>
                      ...
                      <select
                        style={{width:'100%',padding:'9px 12px',border:'1.5px solid #e2e8f0',borderRadius:8,fontSize:13,color:(imputados.length===0?c.correo_notificacion:imputados[0].correo_notificacion)?'#2F5D48':'#94a3b8',background:'#fff',cursor:'pointer',...f}}
            ...
            {activeTab==='audiencias'&&(
              <div>
                ...
                {showAudForm&&(
                  <div style={{background:'#FAF7F0',border:'1.5px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:14}}>
            ...
            {activeTab==='top'&&(
              <div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:c.tiene_top?'#faf5ff':'#FAF7F0',border:`1.5px solid ${c.tiene_top?'#ddd6fe':'#e2e8f0'}`,borderRadius:12,padding:'16px 20px',marginBottom:16}}>
                  <div>
      ...
      <div style={{maxWidth:1380,margin:'0 auto',padding:'28px'}}>
        ...
        <div style={{display:'flex',gap:16,marginBottom:24}}>
          {[
            ...
            return(<div key={st.key} className="stat-card" onClick={()=>{
            ...
            }} style={{flex:1,textAlign:'center',background:enfasis?'#2F5D48':'#fff',border:'none',borderRadius:20,padding:'28px 22px',boxShadow:enfasis?'0 8px 24px rgba(30,58,47,0.16)':'0 1px 3px rgba(15,23,42,0.06)',transition:'all 0.2s'}}>
              ...
              <div style={{fontSize:38,fontWeight:800,color:enfasis?'#fff':'#2F5D48',lineHeight:1,letterSpacing:'-1.5px',...f}}>{st.num}</div>
            </div>)
          })}
        </div>
        ...
        {showStats&&(
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:16,padding:28,marginBottom:24}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:10,marginBottom:8}}>
              <div>
                <div style={{fontSize:17,fontWeight:800,color:'#2F5D48',...f}}>Estadísticas del portfolio</div>
                <div style={{fontSize:12,color:'#94a3b8',marginTop:4,...f}}>
                  {hayFiltrosActivos ? <>Mostrando <strong style={{color:'#2F5D48'}}>{filtered.length}</strong> causa{filtered.length!==1?'s':''} con los filtros activos</> : <>Mostrando las {filtered.length} causas del portfolio (sin filtros)</>}
                </div>
              </div>
                ...
                <ResponsiveContainer width="100%" height={isMobile?240:320}>
                  <BarChart data={chartDelitos} layout="vertical" margin={{left:isMobile?0:8,right:isMobile?12:24,top:4,bottom:4}}>
                    ...
                    <Bar dataKey="value" radius={[0,6,6,0]} cursor="pointer" onClick={(data)=>setFilterDelito(prev=>prev===data.nombreCompleto?'':data.nombreCompleto)}>
                      {chartDelitos.map((d,i)=><Cell key={i} fill={COLORS[i%COLORS.length]} stroke={filterDelito===d.nombreCompleto?'#2F5D48':'none'} strokeWidth={filterDelito===d.nombreCompleto?2:0}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                )}
                ...
                <ResponsiveContainer width="100%" height={isMobile?240:320}>
                  <BarChart data={chartTribunales} layout="vertical" margin={{left:isMobile?0:8,right:isMobile?12:24,top:4,bottom:4}}>
                    ...
                    <Bar dataKey="value" radius={[0,6,6,0]} cursor="pointer" onClick={(data)=>setFilterTribunal(prev=>prev===data.name?'':data.name)}>
                      {chartTribunales.map((d,i)=><Cell key={i} fill={COLORS[i%COLORS.length]} stroke={filterTribunal===d.name?'#2F5D48':'none'} strokeWidth={filterTribunal===d.name?2:0}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                )}
                ...
                <>
                  ...
                  {totalFavorContra > 0 && (
                    <div style={{marginBottom:24}}>
                      ...
                      <div style={{display:'flex',background:'#FAF7F0',borderRadius:8,height:10,overflow:'hidden',marginTop:10,border:'1px solid #e2e8f0'}}>
                  ...
                  {chartResultados.map(r=>(
                    <div key={r.subestado} style={{display:'flex',alignItems:'center',gap:12,marginBottom:8}}>
                      ...
                      <div style={{flex:1,background:'#FAF7F0',borderRadius:6,height:22,position:'relative',overflow:'hidden',border:`1px solid ${r.border}`}}>
                        <div style={{width:`${r.pct}%`,height:'100%',background:r.bg,borderRight:`2px solid ${r.color}`,transition:'width 0.3s'}}/>
                      </div>
                      <div style={{width:70,fontSize:12,fontWeight:700,color:'#2F5D48',textAlign:'right',flexShrink:0,...f}}>{r.value} · {r.pct}%</div>
                    </div>
                  ))}
                </>
              )}
        ...
        <div style={{display:'flex',gap:10,marginBottom:showFiltros?10:24,alignItems:'center',flexWrap:'wrap',justifyContent:'space-between'}}>
          ...
          <div style={{width:320,position:'relative'}}>
            ...
            <input style={{width:'100%',padding:'11px 14px',paddingLeft:38,border:'none',borderRadius:14,fontSize:13,color:'#2F5D48',background:'#fff',boxShadow:'0 1px 2px rgba(15,23,42,0.06)',...f}} placeholder="Buscar por RUC, RIT, imputado, delito..." value={search} onChange={e=>setSearch(e.target.value)}/>
          </div>
        ...
        {showFiltros && (
          <div style={{display:'flex',gap:8,marginBottom:24,flexWrap:'wrap',alignItems:'center',background:'#fff',borderRadius:16,padding:14,boxShadow:'0 1px 3px rgba(15,23,42,0.06)'}}>
            <select style={{width:'auto',minWidth:150,padding:'8px 12px',border:'none',borderRadius:12,fontSize:12,color:'#2F5D48',background:'#FAF7F0',...f}} value={filterTribunal} onChange={e=>setFilterTribunal(e.target.value)}><option value="">Todos los tribunales</option>{tribunales.map(t=><option key={t} value={t}>{t}</option>)}</select>
            <select style={{width:'auto',minWidth:130,padding:'8px 12px',border:'none',borderRadius:12,fontSize:12,color:'#2F5D48',background:'#FAF7F0',...f}} value={filterEstado} onChange={e=>setFilterEstado(e.target.value)}><option value="">Todos los estados</option>{Object.entries(estadoConfig).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select>
            <div style={{width:180}}>
            ...
            <select style={{width:'auto',minWidth:130,padding:'8px 12px',border:'none',borderRadius:12,fontSize:12,color:'#2F5D48',background:'#FAF7F0',...f}} value={filterRegimen} onChange={e=>setFilterRegimen(e.target.value)}>
            ...
            {filtered.map((c)=>(
              <div key={c.id} className="row-hover causa-row" onClick={()=>openCausa(c)} style={{borderRadius:14}}>
                ...
                <div className="causa-col-desktop" style={{display:'grid',gridTemplateColumns:'140px 110px 140px minmax(180px,320px) 1fr',columnGap:24}}>
                  <div style={{padding:'14px 20px',fontSize:12,fontWeight:700,color:'#2F5D48',...f,display:'flex',alignItems:'center',gap:6}}>
                    {c.ruc}
                  ...
                  <div style={{padding:'14px 20px',textAlign:'center',...f}}><div style={{maxWidth:'100%',margin:'0 auto',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontSize:13,color:'#2F5D48',fontWeight:500}}>{(c.imputado||'').replace(/\|/g,' / ')}</div></div>
                ...
                <div className="causa-row-mobile" style={{padding:'12px 14px',margin:'6px 8px',background:c.estado==='vigente'?'#fafffd':'#FAF7F0',border:`2px solid ${c.estado==='vigente'?'#86efac':'#cbd5e1'}`,borderRadius:12}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
                    <span style={{fontSize:13,fontWeight:700,color:'#2F5D48',display:'flex',alignItems:'center',gap:6,...f}}>
                      {c.ruc}
                  ...
                  <div style={{fontSize:13,fontWeight:600,color:'#2F5D48',marginTop:3,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',...f}}>{(c.imputado||'').replace(/\|/g,' / ')}</div>
                </div>
              </div>
            ))}
      ...
      {showNuevaCausa&&(
        <motion.div
          ...
          <motion.div
            ...
            <div style={{fontSize:20,fontWeight:800,color:'#2F5D48',marginBottom:24,...f}}>Nueva Causa</div>
                ...
                <div key={idx} className="grid2-mobile" style={{gridColumn:'1/-1', display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, ...(idx>0 ? {borderTop:'1px dashed #e2e8f0', paddingTop:16, marginTop:2} : {})}}>
                  {nuevaCausa.imputados.length > 1 && (
                    <div style={{gridColumn:'1/-1', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                      <div style={{fontSize:12, fontWeight:700, color:'#2F5D48', ...f}}>Imputado {idx+1}</div>
                      {idx > 0 && (
                  ...
                  <div>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      ...
                      {imp.fecha_nac && (() => {
                        ...
                        return edad !== null ? <span style={{flexShrink:0,fontSize:11,color:'#2F5D48',fontWeight:600,background:'#eff6ff',padding:'4px 9px',borderRadius:10,...f}}>{edad} AÑOS HOY</span> : null
                      })()}
                    </div>
                  </div>
                  <div>
                  ...
                  <div style={{gridColumn:'1/-1'}}>
                    ...
                    {imp.cautelares.map((ct, cIdx) => (
                      <div key={cIdx} style={{marginBottom:8,padding:'10px 12px',background:'#FAF7F0',border:'1px solid #e2e8f0',borderRadius:10,display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap'}}>
                        ...
                        {(TIPOS_ABONO_DIRECTO.includes(ct.tipo) || ct.tipo === CAUTELAR_SENAME) && ct.fecha_inicio && (
                          <div style={{display:'flex',alignItems:'center',gap:6,paddingBottom:9}}>
                            ...
                            <span style={{fontSize:12,fontWeight:700,color:'#2F5D48',...f}}>{diasEntreFechasCaut(ct.fecha_inicio, hoyISO())}d{ct.tipo === CAUTELAR_SENAME ? ' (SENAME)' : ''}</span>
                          </div>
                        )}
