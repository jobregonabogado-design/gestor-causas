...
const CSS = `
  ...
  .btn-secondary:hover { border-color:#93c5fd; color:#2F5D48; background:#FAF7F0; }
...
function PerfilAbogado({ abogado, setAbogado, onGuardar, guardando }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20, marginBottom: 20 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 4, ...f }}>Datos del abogado patrocinante</div>
...
function SelectorDelegado({ delegados, delegadoSel, setDelegadoSel, nuevo, setNuevo }) {
  const inp = { width: '100%', padding: '8px 10px', border: '1.5px solid #E2E8F0', borderRadius: 8, fontSize: 12, color: '#2F5D48', background: '#fff', ...f }
  return (
    <div style={{ background: '#FAF7F0', border: '1px solid #E2E8F0', borderRadius: 12, padding: 16, marginBottom: 20 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#2F5D48', marginBottom: 10, ...f }}>Abogado a quien se delega el poder</div>
    ...
    <div style={{ background: '#FAF7F0', minHeight: '100vh', ...f }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: '#2F5D48', margin: 0, letterSpacing: '-0.5px' }}>Escritos</h1>
        ...
        {/* Paso 1: elegir causa */}
        <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20, marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 12, ...f }}>{pasoCausa}. Elige la causa</div>
          {causaSel ? (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#FAF7F0', border: '1px solid #E2E8F0', borderRadius: 10, padding: '12px 16px' }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', ...f }}>RUC {causaSel.ruc} · RIT {causaSel.rit || '—'}</div>
                ...
                <div style={{ marginTop: 10, border: '1px solid #E2E8F0', borderRadius: 10, overflow: 'hidden' }}>
                  {causas.map(c => (
                    <div key={c.id} className="causa-row" onClick={() => seleccionarCausa(c)} style={{ padding: '10px 14px', borderBottom: '1px solid #F1F5F9' }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: '#2F5D48', ...f }}>RUC {c.ruc} · RIT {c.rit || '—'}</div>
        ...
        {causaSel?.tiene_top && (
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20, marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 12, ...f }}>{pasoDestino}. Esta causa tiene Juicio Oral — ¿a qué tribunal va este escrito?</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={() => setDestinoJudicial('garantia')} style={{ padding: '10px 16px', borderRadius: 10, fontSize: 12, fontWeight: 600, textAlign: 'left', border: `1.5px solid ${destinoJudicial === 'garantia' ? '#2F5D48' : '#E2E8F0'}`, background: destinoJudicial === 'garantia' ? '#FAF7F0' : '#fff', color: '#2F5D48', cursor: 'pointer', ...f }}>
              ...
              <button onClick={() => setDestinoJudicial('top')} style={{ padding: '10px 16px', borderRadius: 10, fontSize: 12, fontWeight: 600, textAlign: 'left', border: `1.5px solid ${destinoJudicial === 'top' ? '#2F5D48' : '#E2E8F0'}`, background: destinoJudicial === 'top' ? '#FAF7F0' : '#fff', color: '#2F5D48', cursor: 'pointer', ...f }}>
        ...
        {causaSel && imputados.length > 1 && (
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20, marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 12, ...f }}>{pasoImputados}. Esta causa tiene varios imputados — ¿respecto de quién es el escrito?</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {imputados.map(imp => {
                const marcado = impsSel.some(i => i.id === imp.id)
                return (
                  <button key={imp.id} onClick={() => toggleImputado(imp)} style={{ padding: '8px 16px', borderRadius: 20, fontSize: 12, fontWeight: 600, border: `1.5px solid ${marcado ? '#2F5D48' : '#E2E8F0'}`, background: marcado ? '#2F5D48' : '#fff', color: marcado ? '#fff' : '#64748b', cursor: 'pointer', ...f }}>
        ...
        {causaSel && (imputados.length <= 1 || impsSel.length > 0) && (
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20, marginBottom: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 12, ...f }}>{pasoEscritos}. Elige el o los escritos</div>
              ...
              {plantillas.map(p => {
                ...
                return (
                  <div key={p.id} className="plantilla-card" onClick={() => toggleCapitulo(p)}
                    style={{ border: `1.5px solid ${marcada ? '#2F5D48' : '#E2E8F0'}`, borderRadius: 12, padding: 16, background: marcada ? '#FAF7F0' : '#fff' }}>
                    {marcada && (
                      <div style={{ position: 'absolute', top: 10, right: 10, width: 22, height: 22, borderRadius: '50%', background: '#2F5D48', color: '#fff', fontSize: 11, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', ...f }}>{posicion + 1}</div>
                    )}
                    <div style={{ fontSize: 20, marginBottom: 8 }}>📄</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 4, ...f }}>{p.nombre}</div>
        ...
        {escrito && (
          <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 14, padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#2F5D48', ...f }}>Revisa y edita antes de descargar</div>
              ...
              <div style={{ border: '1.5px solid #E2E8F0', borderRadius: 10, padding: '14px 20px', marginBottom: 10 }}>
                ...
                {escrito.preSumaLineas.map((linea, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    {linea.rotulo && <span style={{ fontSize: 12, fontWeight: 700, color: '#2F5D48', flexShrink: 0, fontFamily: "'Times New Roman',serif" }}>{linea.rotulo}:</span>}
            ...
            <div style={{ border: '1.5px solid #E2E8F0', borderRadius: 10, padding: '16px 20px', marginBottom: 2 }}>
              {escrito.sumaLineas.map((linea, i) => (
                <input key={i} value={linea} onChange={e => setEscrito(prev => ({ ...prev, sumaLineas: prev.sumaLineas.map((l, j) => j === i ? e.target.value : l) }))}
                  style={{ width: '100%', border: 'none', outline: 'none', fontSize: 13, fontWeight: 700, color: '#2F5D48', marginBottom: 4, padding: '2px 0', fontFamily: "'Times New Roman',serif" }} />
              ))}
              <input value={escrito.destinatario} onChange={e => setEscrito(prev => ({ ...prev, destinatario: e.target.value }))}
                style={{ width: '100%', border: 'none', outline: 'none', fontSize: 13, fontWeight: 700, color: '#2F5D48', textAlign: 'center', marginTop: 10, padding: '2px 0', fontFamily: "'Times New Roman',serif" }} />
            </div>
            ...
            <textarea value={escrito.cuerpo} onChange={e => setEscrito(prev => ({ ...prev, cuerpo: e.target.value }))}
              style={{ width: '100%', minHeight: 380, padding: 20, border: '1.5px solid #E2E8F0', borderRadius: 10, fontSize: 13, lineHeight: 1.7, color: '#2F5D48', fontFamily: "'Times New Roman',serif", textAlign: 'justify', resize: 'vertical' }} />
