    ...
    <div style={{ background:'#FAF7F0', minHeight:'calc(100vh - 60px)', ...f }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:10 }}>
          <div>
            <div style={{ fontSize:20, fontWeight:800, color:'#2F5D48', letterSpacing:'-0.4px' }}>Códigos y Leyes</div>
          ...
          <button onClick={actualizar} disabled={actualizando}
            style={{ fontFamily:"'Manrope','Inter',sans-serif", background:'#fff', border:'1.5px solid #e5e7eb', borderRadius:10, padding:'8px 18px', fontSize:13, fontWeight:600, cursor:'pointer', color:'#374151' }}>
                ...
                <div key={c.id} style={{ position:'relative' }}>
                  ...
                  {tieneTextoCargado(c) && (
                    <button onClick={()=>setSeleccionId(c.id)}
                      style={{ position:'absolute', bottom:8, right:8, fontFamily:"'Manrope','Inter',sans-serif", background:'#fff', border:'1px solid #e2e8f0', borderRadius:8, padding:'5px 10px', fontSize:10, fontWeight:600, color:'#374151', cursor:'pointer' }}>
                      📄 Texto
                    </button>
                  )}
                  {!c.fuente_url && (
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.5)', display:'flex', alignItems:'center', justifyContent:'center', padding:20, zIndex:200 }} onClick={e=>e.target===e.currentTarget && setSeleccionId(null)}>
          <div style={{ background:'#fff', borderRadius:16, maxWidth:800, width:'100%', maxHeight:'85vh', overflowY:'auto', padding:24 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <div style={{ fontSize:16, fontWeight:800, color:'#2F5D48' }}>{seleccionado.titulo}</div>
            ...
            <div style={{ fontSize:14, lineHeight:1.8, color:'#2F5D48', whiteSpace:'pre-wrap' }}>{seleccionado.contenido}</div>
          </div>
        </div>
      )}
    </div>
